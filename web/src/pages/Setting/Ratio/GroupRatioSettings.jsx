...
export default function GroupRatioSettings(props) {
  ...
  const handleGroupTableChange = useCallback(
    ({ GroupRatio, UserUsableGroups, UserVisibleGroups, GroupBillingType }) => {
      setInputs((prev) => ({
        ...prev,
        GroupRatio,
        UserUsableGroups,
  ...
  const handleSpecialVisibleChange = useCallback((value) => {
    setInputs((prev) => ({
      ...prev,
      'group_ratio_setting.group_special_visible_group': value,
    }));
  }, []);
    ...
    <Form key='form-visual' values={inputs} style={{ marginBottom: 15 }}>
      <Form.Section text={t('分组管理')}>
        ...
        <GroupTable
          ...
          userVisibleGroups={inputs.UserVisibleGroups}
          groupBillingType={inputs.GroupBillingType}
      ...
      <Form.Section text={t('分组特殊可见分组')}>
        ...
        >
          {t(
            '为特定用户分组配置创建令牌时可见分组的增减规则。未配置时默认沿用可访问分组。',
          )}
        </Text>
        <GroupSpecialUsableRules
          key={`gsv_${dv}`}
          value={inputs['group_ratio_setting.group_special_visible_group']}
          groupNames={groupNames}
          onChange={handleSpecialVisibleChange}
        />
      </Form.Section>
    </Form>
  );
      ...
      <Form.Section text={t('分组JSON设置')}>
        ...
        <Row gutter={16}>
          <Col xs={24} sm={16}>
            <Form.TextArea
              ...
              extraText={t(
                '请求鉴权允许访问的分组，旧令牌是否还能继续调用由这里决定，格式为 JSON 字符串，例如：{"vip": "VIP 用户", "test": "测试"}',
              )}
        ...
        <Row gutter={16}>
          <Col xs={24} sm={16}>
            <Form.TextArea
              ...
              placeholder={t('为一个 JSON 文本，键为分组名称，值为分组描述')}
              extraText={t(
                '创建或编辑令牌时展示的分组。留空时默认等于用户可访问分组；配置后可以隐藏仍可访问的旧分组。',
              )}
              field={'UserVisibleGroups'}
              autosize={{ minRows: 6, maxRows: 12 }}
              trigger='blur'
              stopValidateWithError
              rules={[
                {
                  validator: (rule, value) =>
                    !value || value.trim() === '' || verifyJSON(value),
                  message: t('不是合法的 JSON 字符串'),
                },
              ]}
              onChange={(value) =>
                setInputs((prev) => ({ ...prev, UserVisibleGroups: value }))
              }
            />
          </Col>
        </Row>
        <Row gutter={16}>
          <Col xs={24} sm={16}>
            <Form.TextArea
              ...
              extraText={t(
                '键为用户分组名称，值为操作映射对象。用于控制创建或编辑令牌时展示哪些分组；留空时沿用可访问分组。',
              )}
              field={'group_ratio_setting.group_special_visible_group'}
              autosize={{ minRows: 6, maxRows: 12 }}
              trigger='blur'
              stopValidateWithError
              rules={[
        ...
        <Row gutter={16}>
          <Col xs={24} sm={16}>
            <Form.TextArea
              label={t('分组扣费类型')}
              placeholder={t('为一个 JSON 文本，键为分组名称，值为扣费类型')}
              extraText={t(
                '支持 default（保持现状）、subscription_only（仅订阅）、wallet_only（仅余额）。未配置的分组视为 default。',
              )}
              field={'GroupBillingType'}
              autosize={{ minRows: 6, maxRows: 12 }}
              trigger='blur'
              stopValidateWithError
              rules={[
              ...
              onChange={(value) =>
                setInputs((prev) => ({ ...prev, GroupBillingType: value }))
              }
            />
          </Col>
        </Row>

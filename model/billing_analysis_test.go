...
func TestGetBillingAnalysisBuildsTokenMetrics(t *testing.T) {
	truncateTables(t)
	...
	assert.Equal(t, int64(140), result.Summary.TokenMetrics.InputTokens)
	...
	assert.Equal(t, int64(250), result.Summary.TokenMetrics.TotalTokensWithCache)
	assert.InDelta(t, 140.0/250.0, result.Summary.TokenMetrics.InputShare, 0.000001)
	assert.InDelta(t, 110.0/250.0, result.Summary.TokenMetrics.CompletionShare, 0.000001)
	assert.InDelta(t, 50.0/140.0, result.Summary.TokenMetrics.CacheShare, 0.000001)
	assert.InDelta(t, 70.0, result.Summary.TokenMetrics.AvgInputTokensPerRequest, 0.000001)
	...
	assert.Equal(t, int64(140), tokenRow.TokenMetrics.InputTokens)
	...
	assert.InDelta(t, 70.0, tokenRow.TokenMetrics.AvgInputTokensPerRequest, 0.000001)
...
func TestGetBillingAnalysisSeparatesInputAndCacheWhenInputTotalIncludesCache(t *testing.T) {
	...
	assert.Equal(t, int64(10400), result.Summary.TokenMetrics.InputTokens)
	...
	assert.InDelta(t, 10400.0/10450.0, result.Summary.TokenMetrics.InputShare, 0.000001)
	assert.InDelta(t, 50.0/10450.0, result.Summary.TokenMetrics.CompletionShare, 0.000001)
	assert.InDelta(t, 9100.0/10400.0, result.Summary.TokenMetrics.CacheShare, 0.000001)
}
